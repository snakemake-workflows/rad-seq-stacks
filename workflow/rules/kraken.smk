...
rule download_minikraken_db:
    input:
        HTTP.remote(config["params"]["kraken"]["db_link"],),
    output:
...
rule kraken:
    ...
    params:
        gzip=(
            lambda wildcards, input: "--gzip-compressed"
            if input.reads[0].endswith(".gz")
            else ""
        ),
    shell:
        """
...
rule plot_classification_tree:
    ...
    shell:
        ...
        "-Nfontsize=10 -Npenwidth=2 -Epenwidth=2 -Ecolor=grey -Nbgcolor=white "
        "> {output} 2> {log}"
